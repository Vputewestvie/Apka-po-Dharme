...
function practiceFromRow(row) {
    return new Practice(row.id, row.user_id, row.title, row.description, row.category_id ?? "", row.default_duration_minutes, row.color, row.icon, {
        kind: row.image_kind,
        ref: row.image_ref,
    }, "manual", row.personal_notes, row.is_archived, row.created_at, row.updated_at);
}
function materialToRow(material) {
    return {
        id: material.id,
...
function diaryFromRow(row) {
    return new DiaryEntry(String(row.id), String(row.user_id), String(row.practice_id), String(row.scheduled_practice_id), String(row.kind), String(row.created_at), String(row.updated_at ?? row.created_at), String(row.text ?? ""), row.voice_file_id ? String(row.voice_file_id) : null, row.transcription ? String(row.transcription) : null);
}
...
function scheduleFromRow(row, items) {
    return new Schedule(row.id, row.user_id, row.date, row.created_by, row.title, items);
}
function scheduledPracticeToRow(item, scheduleId) {
    const now = new Date().toISOString();
    return {
        id: item.id,
...
function scheduledPracticeFromRow(row, plannedDate) {
    return new ScheduledPractice(row.id, row.practice_id, plannedDate, row.planned_start_time, row.planned_duration_minutes, row.sort_order, row.status);
}
function practiceSessionToRow(session) {
    const now = new Date().toISOString();
    return {
        id: session.id,
...
function practiceSessionFromRow(row) {
    return new PracticeSession(String(row.id), String(row.scheduled_practice_id), String(row.practice_id), String(row.user_id), Number(row.planned_duration_minutes ?? 0), row.started_at ? String(row.started_at) : null, row.paused_at ? String(row.paused_at) : null, row.finished_at ? String(row.finished_at) : null, Number(row.actual_duration_seconds ?? 0), String(row.status));
}
function practiceSessionToCompletionRow(session) {
    const now = new Date().toISOString();
    return {
        id: session.id,
        scheduled_practice_id: session.scheduledPracticeId,
...
export class SqlitePracticeRepository {
    client;
    constructor(client) {
        this.client = client;
    }
    async listByUserId(userId) {
        const result = await this.client.query("select * from practices where user_id = ?", [userId]);
        return result.rows.map(practiceFromRow);
    }
    async getById(id) {
        const result = await this.client.query("select * from practices where id = ? limit 1", [id]);
        const row = result.rows[0];
        return row ? practiceFromRow(row) : null;
    }
    async upsert(practice) {
        const row = practiceToRow(practice);
        await this.client.execute(`insert into practices (
...
export class SqliteMaterialRepository {
    client;
    constructor(client) {
        this.client = client;
    }
    async listByPracticeId(practiceId) {
        const result = await this.client.query("select * from practice_materials where practice_id = ?", [practiceId]);
        return result.rows.map(materialFromRow);
    }
    async upsert(material) {
        const row = materialToRow(material);
        await this.client.execute(`insert into practice_materials (
...
export class SqliteDiaryRepository {
    client;
    constructor(client) {
        this.client = client;
    }
    async listByUserId(userId) {
        const result = await this.client.query("select * from journal_entries where user_id = ? order by created_at desc", [userId]);
        return result.rows.map(diaryFromRow);
    }
    async upsert(entry) {
        const row = diaryToRow(entry);
        await this.client.execute(`insert into journal_entries (
...
export class SqliteStatisticsRepository {
    client;
    constructor(client) {
        this.client = client;
    }
    async getSnapshot(userId) {
        const completionTotals = await this.client.query(`select
       ...
       where s.user_id = ?`, [userId]);
        const favoriteResult = await this.client.query(`select sp.practice_id, count(*) as total
       from practice_completions pc
       join scheduled_practices sp on sp.id = pc.scheduled_practice_id
       join schedules s on s.id = sp.schedule_id
    ...
    async upsert(snapshot) {
        ...
        await this.client.execute(`insert into statistics_snapshots (
...
export class SqliteNotificationRepository {
    ...
    async listPending(now) {
        const result = await this.client.query("select * from notification_jobs where status = 'pending' and scheduled_at <= ? order by scheduled_at asc", [now]);
        return result.rows;
    }
    async upsert(job) {
        await this.client.execute(`insert into notification_jobs (
    ...
    async markSent(jobId, sentAt) {
        await this.client.execute("update notification_jobs set status = 'sent', sent_at = ?, updated_at = ? where id = ?", [sentAt, sentAt, jobId]);
    }
    async cancel(jobId) {
        const now = new Date().toISOString();
        await this.client.execute("update notification_jobs set status = 'cancelled', updated_at = ? where id = ?", [now, jobId]);
    }
}
export class SqliteScheduleRepository {
    ...
    async getByUserIdAndDate(userId, date) {
        const scheduleResult = await this.client.query("select * from schedules where user_id = ? and date = ? limit 1", [userId, date]);
        const scheduleRow = scheduleResult.rows[0];
        if (!scheduleRow)
            return null;
        const itemsResult = await this.client.query("select * from scheduled_practices where schedule_id = ? order by sort_order asc", [scheduleRow.id]);
        return scheduleFromRow(scheduleRow, itemsResult.rows.map((item) => scheduledPracticeFromRow(item, scheduleRow.date)));
    }
    async upsert(schedule) {
        const row = scheduleToRow(schedule);
        await this.client.execute(`insert into schedules (id, user_id, date, title, created_by, created_at, updated_at)
       values (?, ?, ?, ?, ?, ?, ?)
       on conflict(id) do update set
         date = excluded.date,
         title = excluded.title,
         created_by = excluded.created_by,
         updated_at = excluded.updated_at`, [row.id, row.user_id, row.date, row.title, row.created_by, row.created_at, row.updated_at]);
        ...
        for (const item of schedule.items) {
            ...
            await this.client.execute(`insert into scheduled_practices (
...
export class SqliteTimerRepository {
    ...
    async getByScheduledPracticeId(scheduledPracticeId) {
        const result = await this.client.query(`select ts.*, sp.practice_id, s.user_id
       from timer_sessions ts
       join scheduled_practices sp on sp.id = ts.scheduled_practice_id
       join schedules s on s.id = sp.schedule_id
       where ts.scheduled_practice_id = ?
       limit 1`, [scheduledPracticeId]);
        const row = result.rows[0];
        return row ? practiceSessionFromRow(row) : null;
    }
    async upsert(session) {
        const row = practiceSessionToRow(session);
        await this.client.execute(`insert into timer_sessions (
    ...
    async appendEvent(sessionId, eventType, payloadJson) {
        await this.client.execute("insert into timer_events (id, timer_session_id, event_type, event_at, payload_json) values (?, ?, ?, ?, ?)", [cryptoRandomId(), sessionId, eventType, new Date().toISOString(), payloadJson]);
    }
}
export class SqlitePracticeCompletionRepository {
    client;
    constructor(client) {
        this.client = client;
    }
    async getByScheduledPracticeId(scheduledPracticeId) {
        const result = await this.client.query(`select pc.*, sp.practice_id, s.user_id, ts.planned_duration_minutes
       from practice_completions pc
       join scheduled_practices sp on sp.id = pc.scheduled_practice_id
       join schedules s on s.id = sp.schedule_id
       ...
       limit 1`, [scheduledPracticeId]);
        const row = result.rows[0];
        return row
            ? new PracticeSession(String(row.id), String(row.scheduled_practice_id), String(row.practice_id), String(row.user_id), Number(row.planned_duration_minutes ?? 0), null, null, String(row.completed_at), Number(row.actual_duration_seconds ?? 0), String(row.result), String(row.result), String(row.skip_reason ?? "") || null)
            : null;
    }
    async upsert(completion) {
        const row = practiceSessionToCompletionRow(completion);
        await this.client.execute(`insert into practice_completions (
